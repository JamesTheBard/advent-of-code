...
class Solution:
    ...
    def __init__(self, input_file: Union[str, Path]):
        ...
        self.data: list[str] = [i.strip() for i in input_file.open('r').readlines()]
    ...
    def process_data(self) -> None:
        for idx, line in enumerate(self.data):
            if matches := re.finditer(r'(\d+)', line):
                for m in matches:
                    s: list[Symbol] = self.get_symbols(m.start(), m.end(), idx)
                    self.numbers.append(Number(int(m.group(1)), s))

    def get_symbols(self, x_start: int, x_end: int, y: int) -> list[Symbol]:
    ...
    def solve_part2(self) -> int:
        ...
        for gear in gears:
            numbers: tuple[Number] = tuple(i for i in self.numbers if i.has_gear(gear))
            if len(numbers) == 2:
